...
class WosQuery:
    ...
    async def collect_papers(self, citedcheck=False, limit=20):
        ...
        async with aiohttp.ClientSession(headers=self.headers, connector=conn) as session:
            self.papers = await asyncio.gather(
                *[self.parse_paper(session, self.urlprefix, count + 1, citedcheck=citedcheck) for count in
                  range(self.num_items)])

    async def parse_paper(self, session, prefix, count, citedcheck=False, ocount=0):
        html2 = ""
    ...
    async def main(self, path, citedcheck=False):
        await self.query()
        await self.collect_papers(citedcheck=citedcheck)
        logger.info("all download tasks are finished")
        with open(path, "w") as output:
            json.dump(self.papers, output)
        logger.info("data are written into json file: %s" % path)
